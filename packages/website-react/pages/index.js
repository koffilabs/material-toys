import classes from "./index.module.scss";
import { Button, Card } from "@material-toys/react";
export default function Index() {
  return (
    <div style={{ background: "#ddd" }} className={classes}>
      <h1>Material Toys</h1>
      <h3>A Material You implementation for React - website</h3>
      <p>this is a button</p>
      <Card>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aperiam
          corporis dolorem odio repellat sequi. Ab ad consequatur deserunt dicta
          dolore eligendi, excepturi natus non, officiis, quam quas tenetur
          veniam vero.
        </p>
      </Card>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ad cum
        doloremque excepturi expedita laboriosam magnam quibusdam, quisquam quo
        veniam vero? Aperiam autem dolorum facere impedit iste minima omnis ut.
        Id.
      </p>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ad cum
        doloremque excepturi expedita laboriosam magnam quibusdam, quisquam quo
        veniam vero? Aperiam autem dolorum facere impedit iste minima omnis ut.
        Id.
      </p>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ad cum
        doloremque excepturi expedita laboriosam magnam quibusdam, quisquam quo
        veniam vero? Aperiam autem dolorum facere impedit iste minima omnis ut.
        Id.
      </p>
    </div>
  );
}
