...
describe("button style isolated unit tests suite", () => {
  test("should apply MdSysTypescaleLabelLargeLineHeight", () => {
    ...
    expect(
      Button({ ...tokens, MdSysTypescaleLabelLargeLineHeight: value }, {})
        .lineHeight
    ).toBe(`${value}px`);
  });
  test("should apply MdSysTypescaleLabelLargeSize", () => {
    const value = Math.random();
    expect(
      Button({ ...tokens, MdSysTypescaleLabelLargeSize: value }, {}).fontSize
    ).toBe(value);
  });
  test("should apply MdSysTypescaleLabelLargeWeight", () => {
    const value = Math.random();
    expect(
      Button({ ...tokens, MdSysTypescaleLabelLargeWeight: value }, {})
        .fontWeight
    ).toBe(value);
  });
  test("should apply MdSysTypescaleLabelLargeFont", () => {
    ...
    expect(
      Button({ ...tokens, MdSysTypescaleLabelLargeFont: value }, {}).fontFamily
    ).toBe(value);
  });
  test("should apply MdSysTypescaleLabelLargeTracking", () => {
    const value = Math.random();
    expect(
      Button({ ...tokens, MdSysTypescaleLabelLargeTracking: value }, {})
        .letterSpacing
    ).toBe(value);
  });
  test("should apply MdSysColorPrimary - Light variation", () => {
    const value = "white";
    expect(
      Button({ ...tokens, MdSysColorPrimary: value }, {})["&>.state"]
        .backgroundColor
    ).toBe(value);

    expect(
      Button({ ...tokens, MdSysColorPrimary: value }, {})["&.elevated"].color
    ).toBe(value);
    expect(
    ...
    expect(
      Button({ ...tokens, MdSysColorPrimary: value }, {})["&.outlined"].fill
    ).toBe(value);
    expect(
